		...
		if (mPermissionIntent == null) {
			...
			if (context != null) {
				...
				if (Build.VERSION.SDK_INT >= 31) {
					// UsbManager.requestPermission needs to inject result extras into the PendingIntent.
					// Keep it mutable on Android 12+ or the broadcast arrives without the USB payload.
...
//		if (DEBUG) XLogWrapper.v(TAG, "requestPermission:device=" + device);
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (DEBUG) {
				final UsbDevice receiverDevice = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
				XLogWrapper.i(
						TAG,
						"mUsbReceiver action=" + action
